...
export class ReportsService {
  ...
  private cacheKey(...parts: (string | number | undefined)[]): string {
    return this.cache?.key('reports', ...parts) ?? `reports:` + parts.filter((p) => p !== undefined && p !== null).join(':');
  }
  ...
  async getForecast(pipelineId?: number, period = 'month') {
    return this.cached(this.cacheKey('getForecast', pipelineId, period), async () => {
  ...
  async getVelocity(pipelineId?: number, status?: 'won' | 'lost' | 'all') {
    return this.cached(this.cacheKey('getVelocity', pipelineId, status), async () => {
