...
export default function AdminCommunicationChatPage() {
  ...
  useEffect(() => {
    if (channelQuery && allChannels.length) {
      ...
      if (d) setConversation({ kind: 'dm', id: d.id, name: d.name || `DM ${d.id}`, avatar: d.avatar_url });
    }
  }, [channelQuery, dmQuery, allChannels, allDms]);
  ...
  const handleReaction = (message: Message, emoji: string) => {
    ...
    if (existing) {
      removeReaction(token, message.id, emoji).then(() => queryClient.invalidateQueries({ queryKey: ['messages', token, conversation?.kind, conversation?.id] }));
    } else {
      addReaction(token, message.id, emoji).then(() => queryClient.invalidateQueries({ queryKey: ['messages', token, conversation?.kind, conversation?.id] }));
    }
  };
  ...
  const onSelectDm = (d: DirectMessage) => {
    const other = d.participants?.find((p) => p.user_id !== user?.id)?.user;
    const name = d.name || (other ? `${other.first_name} ${other.last_name}` : `DM ${d.id}`);
    setConversation({ kind: 'dm', id: d.id, name, avatar: d.avatar_url || other?.avatar });
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-[72px_260px_1fr] gap-0 rounded-2xl overflow-hidden nm-raised h-[calc(100vh-10rem)]">
        {/* Workspace rail */}
        <div className="hidden md:flex flex-col items-center py-4 space-y-3 bg-muted/30 border-r border-border/50">
        ...
        {/* Channel / DM sidebar */}
        <div className="hidden md:flex flex-col border-r border-border/50 bg-background/50">
              ...
              {allDms.map((d) => {
                ...
                return (
                  <button key={d.id} onClick={() => onSelectDm(d)} className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm transition-all ${active ? 'nm-inset-sm font-medium' : 'hover:nm-raised-sm'}`}>
                    <Avatar src={d.avatar_url || other?.avatar} fallback={name} className="w-5 h-5 text-[10px]" /> <span className="truncate">{name}</span>
                  </button>
                );
              })}
        ...
        <div className="flex flex-col min-h-0 bg-background">
          <div className="px-4 py-3 border-b border-border/50 flex items-center gap-2 bg-muted/20">
            {conversation ? (
              <>
                {conversation.kind === 'channel' ? <Hash className="h-5 w-5 text-muted-foreground" /> : <Avatar src={(conversation as any).avatar} fallback={conversation.name} className="w-6 h-6 text-[10px]" />}
            ...
            {loadingMessages ? <Skeleton className="h-8 w-3/4" /> : (messages?.messages || []).map((m: Message, idx: number, arr: Message[]) => {
              ...
              return (
                <div key={m.id} className={`group flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                  {showAvatar ? <Avatar src={m.author?.avatar} fallback={m.author ? `${m.author.first_name} ${m.author.last_name}` : `User ${m.user_id}`} className="w-9 h-9 text-xs" /> : <div className="w-9" />}
                  <div className={`max-w-[80%] min-w-0 ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                    {showAvatar && (
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-semibold">{m.author ? `${m.author.first_name} ${m.author.last_name}` : `User ${m.user_id}`}</span>
                    ...
                    {!!m.reactions?.length && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {m.reactions.map((r) => {
                          const reactedByMe = r.user_id === user?.id;
                          return (
                            <button key={r.id} onClick={() => handleReaction(m, r.emoji)} className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-xs border transition-all ${reactedByMe ? 'bg-primary/10 border-primary' : 'bg-muted border-transparent hover:border-primary'}`}>
                              <span>{r.emoji}</span>
                              <span className="text-[10px]">{/* count if aggregated? */}</span>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    {!editingId && (
                      <div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="relative">
                          <button className="p-1 rounded-md text-muted-foreground hover:text-foreground" title="Add reaction"><Smile className="h-3.5 w-3.5" /></button>
                          <div className="absolute bottom-full left-0 mb-1 hidden group-hover/reactions:block">{/* not easy */}</div>
                        </div>
                        {isMe && (
                          <>
                    ...
                    {!editingId && (
                      <div className="flex items-center gap-0.5 mt-1">
                        {COMMON_EMOJIS.map((emoji) => (
                          <button key={emoji} onClick={() => handleReaction(m, emoji)} className="text-xs p-0.5 rounded hover:bg-muted transition-all">{emoji}</button>
                        ))}
                      </div>
                    )}
                  </div>
